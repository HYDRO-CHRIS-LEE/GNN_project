...
class ChebConvAttention(MessagePassing):
    ...
    def forward(
        ...
        TAx_0 = torch.matmul(
            (torch.eye(num_nodes).to(edge_index.device) * spatial_attention).permute(0, 2, 1),x,) #for example (32, 307, 1)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # print("Spatial Vs shape", self._Vs)
...
class TemporalAttention(nn.Module):
    ...
    def __init__(self, in_channels: int, num_of_vertices: int, num_of_timesteps: int):
        ...
        self._be = nn.Parameter(torch.FloatTensor(1, num_of_timesteps, num_of_timesteps)) # for example (1,12,12)
...
class ASTGCNBlock(nn.Module):
    ...
    def forward(
        ...
        X = self._layer_norm(F.leaky_relu(X + X_hat).permute(0, 3, 2, 1))
        X = X.permute(0, 2, 3, 1)
